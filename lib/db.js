...
export async function createNote(data) {
  const sql = getSQL();
  const { title, amount, type, category, description, note_date, tags } = data;
  const rows = await sql`
    INSERT INTO financial_notes (title, amount, type, category, description, note_date, tags)
    VALUES (
      ${title},
      ${amount || 0},
      ${type || 'expense'},
...
export async function deleteNote(id) {
  const sql = getSQL();
  await sql`DELETE FROM financial_notes WHERE id = ${id}`;
}
...
export async function getNoteStats() {
  ...
  const result = { income: { count: 0, total: 0 }, expense: { count: 0, total: 0 } };
  stats.forEach(r => {
    if (r.type === 'income') result.income = { count: Number(r.count), total: Number(r.total) };
    if (r.type === 'expense') result.expense = { count: Number(r.count), total: Number(r.total) };
  });
  result.net = result.income.total - result.expense.total;
  result.by_category = byCategory.map(r => ({ ...r, total: Number(r.total) }));
